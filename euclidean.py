...
# Load songs data from CSV file, from the merge-list
#The train data
...
#Load csv file, from spotify 1.2M songs
#The test data
...
# This because the result were at very high value and to scale down
min_vals = np.min(selected_song_features, axis=0)
max_vals = np.max(selected_song_features, axis=0)
selected_song_features = (selected_song_features - min_vals) / (max_vals - min_vals)
recommended_song_features = (recommended_song_features - min_vals) / (max_vals - min_vals)

# Calculate similarities between selected songs and recommended songs using Euclidean distance
similarities = euclidean_distances(selected_song_features, recommended_song_features)
print(similarities)

top_similar_indices = similarities.argsort()[0][:100] 
...
#Playlist name in the current users spotify
playlist_name = "Euclidean 160BPM new- D,V,S"
playlist = sp.user_playlist_create(user=user_id, name=playlist_name, public=False)

# Add tracks to the playlist
track_uris = [track['id'] for track in filtered_recommendations]  
